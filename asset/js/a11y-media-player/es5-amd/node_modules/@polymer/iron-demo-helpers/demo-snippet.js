...
*/(0,_polymerFn.Polymer)({_template:(0,_htmlTag.html)(_templateObject_1a003cd0f72311e98498d7f727000406()),is:"demo-snippet",properties:{/**
     * Fired when the demo-snippet is ready, i.e. when it has injected the code to demo 
     * in the DOM and it can be interacted with
     * 
     * @event dom-ready
     */ /** @private */_markdown:{type:String}},attached:function attached(){this._observer=(0,_polymerDom.dom)(this.$.content).observeNodes(function(info){this._updateMarkdown()}.bind(this))},detached:function detached(){if(this._observer){(0,_polymerDom.dom)(this.$.content).unobserveNodes(this._observer);this._observer=null}},_updateMarkdown:function _updateMarkdown(){var template=(0,_polymerDom.dom)(this).queryDistributedElements("template")[0];// If there's no template, render empty code.
...
(0,_polymerDom.dom)(this.$.content).unobserveNodes(this._observer);this._observer=null;(0,_polymerDom.dom)(this).appendChild(document.importNode(template.content,!0))}this.dispatchEvent(new CustomEvent("dom-ready"))},_copyToClipboard:function _copyToClipboard(){// From
// https://github.com/google/material-design-lite/blob/master/docs/_assets/snippets.js
var snipRange=document.createRange();snipRange.selectNodeContents(this.$.code);var selection=window.getSelection();selection.removeAllRanges();selection.addRange(snipRange);var result=!1;try{result=document.execCommand("copy");this.$.copyButton.textContent="done"}catch(err){// Copy command is not available
console.error(err);this.$.copyButton.textContent="error"}// Return to the copy button after a second.
setTimeout(this._resetCopyButtonState.bind(this),1e3);selection.removeAllRanges();return result},_resetCopyButtonState:function _resetCopyButtonState(){this.$.copyButton.textContent="copy"}})});